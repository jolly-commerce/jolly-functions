...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    for (let payment of ALLPAYMENTS) {
      ...
      const shopifyOrderName = payment.description;
      ...
      await Shopify_createOrderTransaction(
        config,
        order.id.replace("gid://shopify/Order/", ""),
        {
          kind: "capture",
          amount: payment.amount.toString(),
          parent_id: getTransactionsResponse.transactions.find(
            (t) => !t.parent_id
          )?.id as number,
      ...
      // update custom attributes to add infos
      updateOrderCustomAttributes(config, order, {
        payments: payment as any as GCL_Payment,
      });
    }
...
async function Shopify_getOrder(
  ...
  if ((result as any).data.orders.edges.length > 0) {
    return (result as any).data.orders.edges[0].node;
  }
    ...
    input: {
      ...
      customAttributes: [
        ...existingCustomAttributes,
        {
          ...
          }€ ; ${paymentInfo.payments.charge_date}`,
        },
      ],
    },
  });
